	...
	<div class="placeOrder_modal">
			...
			<div class="placeOrder_box" v-show="oderter">
				...
				<div class="placeOrder_yes">
					<div class="placeOrder_top placeOrder_yes">
						<p>买入【No】</p>
						<p>卖出【No】</p>
						<span class="placeOrder_close"><img @click="hideIndex" src="../../assets/img/eventGroup/event_close.png"/></span>
					</div>
					<div class="placeOrder_main">
						...
						<table class="eventD_price_buy" border="0" cellspacing="0" cellpadding="0">
							...
							<tbody class="buy_best_price">
								<tr v-for="item in buyPrice" :key="item.count" :sellPrice="item">
									<td>{{item.price=='--'?'--':100-item.price|decimal|market}}</td>
									<td>{{item.count}}</td>
								</tr>
							</tbody>
					...
					<div class="placeOrder_main_text placeOrder_main_quick">
						...
						<p class="placeOrder_price" v-if='Toggleshow'>
							<span v-if="n==3">买入价格：</span>
							<span v-if="n==4">卖出价格：</span>
							<input type="text" v-model="currentPrice" />
						</p>
						<div class="placeOrder_num">
							<span v-if="n==3">买入数量：</span>
							<span v-if="n==4">卖出数量：</span>
						...
						<!--0是评鉴   1是预测-->
						<p class="placeOrder_predict" v-show="Toggleshow" v-if="eventTop.eventType==1 && n==3">
							如果预测成功，预计赚
							<i>{{yuceProfit|decimal}}</i>
							<img src="../../assets/img/eventGroup/event_jindou.png" />
						</p>
						<p v-if="eventTop.eventType==0"></p>
		...
		<div class="confirm_box" v-if="isBuyModal">
			<div class="confirm_main_or">
				...
				<div class="buttona" v-show="buttonhide" @click="confir(2)"><button>取消</button></div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			console.log(JSON.stringify(this.eventTop))
			this.getsellPrice();
		...
		methods: {
			...
			getsellPrice() {
				...
				if(currentPricee == "--") {
					...
					this.currentPrice = this.decimal(100 - currentPricee);
				}
			...
			getsure(n) {
				...
				if(this.counter <= 0) {
					...
					if(this.Toggleshow){
						var curPrice = Number(this.currentPrice).toFixed(3)
						this.currentPrice = curPrice.substring(0,curPrice.lastIndexOf('.')+3) 
					...
					this.$emit('update:sellNo', true)
					this.$emit('update:buyYes', false)
					if(this.isquickly){
	                  this.currentPrice='1'
	                }
	                this.$CPOST('/order/v1/order/saveOrder',{
							...
							placePrice: this.currentPrice == "--" ? "0" : this.currentPrice,
		...
		computed:{
			yuceProfit(){
				if(parseInt(this.currentPrice) > 0){
					return (100-this.currentPrice)*this.counter;					
				}else{
					return 0;
				}
...
<style scoped>
	...
	.eventD_price_buy {
		...
		width: 49%;
		text-align: center;
		/* background: url(../../assets/img/fenge.png) no-repeat center 60px;*/
		float: left;
	}
