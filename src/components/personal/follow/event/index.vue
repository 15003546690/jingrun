	        ...
	        <div class="forecast_content_right">
	          ...
	          <div class="content_right_time">
	            <p class="content_left_time">
	                <span v-if="item.eventType==0" style="background: #4968ab;">评鉴</span>
	                <span v-if="item.eventType==1">预测</span>
...
export default {
  ...
  methods: {
	...
	userAttentionEvent () {
      ...
      this.$Bury(window.location.href,'',0,'pingjiantab_04004')//链接，事件id,类型（ 0-默认 1-活动 2-题材组 3-题材 4-专题），名称
   },
   handleSizeChange(val) {
    ...
    handleCurrentChange(val) {
	  ...
	  showModal(itemId,ind,eventType) {
    	this.eventId = itemId;
    ...
    pushDetail(eventType,eventId) {
    	...
    	if(this.eventReslutType==0){
				this.$router.push({path: '/group/' + this.eventId})
			}else{
...
.content_left_time>a{height: 26px;line-height: 26px;font-size: 14px;color: #888888;margin-left: 6px;float: left;}
.content_right_fllow{font-size: 14px;color: #3282ff;float: right;margin-right: 25px;cursor: pointer;}
.content_right_fllow:hover a{text-decoration: underline;}
