	...
	<div>
		<div class="activityD_main_eventL">
			<ul class="index_container_list">
				<li class="index_container_detail index_yuce" v-for="item in detail.groups" :key="item.groupId" @click='titlee(item.groupId,item.groupPurchaseType,item.groupEventType)' v-if="len>1?true:false">
				<!-- {{item.groupEventType}}。。。{{item.groupPurchaseType}} -->
				    <div class="index_detail_top">
				      <p class="index_detail_top_text">{{item.groupName}}</router-link> <!-- :data-id='item.groupId' :data-groupEventType='item.groupEventType' :data-groupPurchaseType='item.groupPurchaseType' -->
				    ...
				    <div class="index_detail_bottom">
				      ...
				      <div class="index_detail_bottom_main"  v-show='item.groupStatus != 4 && item.groupEventType==0 && item.groupPurchaseType==0'>
						...
						<div class="index_detail_theme" v-else>
				          ...
				          <div class="index_detail_theme_right">
				            <span class="index_detail_price">{{ 100 - item.eventList[0].currentPrice }}</span>
		    		...
		    		if(res.resultValue.activityStatus!=0){
		    			...
		    			if(res.resultValue.join==true){
		    				...
		    				if(res.resultValue.free!=0){//付费
		    					...
		    					this.$CPOST('/activity/v1/activity/payTicket',{
									activityId:this.$route.params.id
								},(res)=>{
									this.alert=true;
