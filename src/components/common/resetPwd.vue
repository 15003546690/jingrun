	...
	<div class="nano-register">
		<div>
			...
			<div class="login-modal-main-bb">
				<p><img class="close" @click="hideResetPwd" src="../../assets/img/login/close1.png"/></p>
			</div>
				...
				<div class="pwd-modal-main-body ">
					<form id="password_form" autocomplete="off">
						<div class="pwd-modal-body-input">
							<div class="change-p1 select-list-limit ">
								<span class="register-country-chongzhi">
									...
									<ul class="register-select-list password-country"></ul>
								</span>
								<input maxlength="11" type="text" class="change-input1" placeholder="请输入手机号" v-model='phoneText'/>
								<span class="change-ti"></span>
							</div>
							<div>
								<input maxlength="6" type="text" class="change-input2" placeholder="请输入验证码"  v-model='yzmText'/>
								<span class="change-ti"></span>
								<i class="shield-write"></i>
								<span class="change-send-code" @click='sendPwd'>发送验证码</span><!--register_zhihui  点击之后-->
							</div>
							<div>
						...
						<div class="change-pwd-btn">
							<button @click='commitPwd'>提交</button>
		...
		methods: {
			...
			sendPwd(){
				if(this.phoneText==''){
					...
					this.$CPOST('/message/communication/v1/sendSMS'+this.$signParam(),{
						"countryCode": "86",
							...
							if(this.pwdText!=this.pwddText){
								...
								this.$CPOST('/account/v1/userInfo/retrievePassWord',{
									...
									if(res.resultCode==200){
										...
										document.querySelector('.change-send-code').innerHTML ='发送验证码';
										this.editState = true;
									}else{
...
.change-input1,.change-input2,.change-input3,.change-input4{
	width: 330px;
	height: 20px;
	line-height: 20px;
	border: 0;
